...
poutput="`/bin/sqlplus64 -s cdr/cdr123@dal-racp008:2032/bell.dre.cdr <<EOF
...
exit;
EOF`"
sed -i '1s/^/IMSI ,MSISDN ,NOTIFICATION_TIME ,NOTIFICATION_TEXT_EN.  /' /home/dreadmin/scripts/presult.csv ;
echo "Hello Gord,

Please find the attached PolicyException error occurred while sending SMS for the date `date '+%Y-%m-%d' -d '24 hour ago'`" | mailx -s "Speed pass purchase exceptions: `date '+%Y-%m-%d' -d '24 hour ago'`" -a /home/dreadmin/scripts/presult.csv "raju.john@syniverse.com" ;
...
soutput="`/bin/sqlplus64 -s cdr/cdr123@dal-racp008:2032/bell.dre.cdr <<EOF
...
set trimspool on;
spool sresult.csv;
select IMSI, MSISDN, START_DATE_TIME as NOTIFICATION_TIME, NOTIFICATION_TEXT_EN from MC_SMS_NOTIFICATION_BELL where start_date_time > sysdate - 2 and trunc(start_date_time) = trunc(sysdate - 1) and NOTIFICATION_STATUS=6
/
spool off;
exit;
EOF`"
sed -i '1s/^/IMSI ,MSISDN ,NOTIFICATION_TIME ,NOTIFICATION_TEXT_EN.  /' /home/dreadmin/scripts/sresult.csv ;
echo "Hello Gord,

Please find the attached ServiceException error occurred while sending SMS for the date `date '+%Y-%m-%d' -d '24 hour ago'`" | mailx -s "Speed pass purchase exceptions: `date '+%Y-%m-%d' -d '24 hour ago'`" -a /home/dreadmin/scripts/sresult.csv "raju.john@syniverse.com" ;
...
set trimspool on;
spool wresult.csv;
select IMSI, MSISDN, START_DATE_TIME as NOTIFICATION_TIME, NOTIFICATION_TEXT_EN from MC_SMS_NOTIFICATION_BELL where start_date_time > sysdate - 2 and trunc(start_date_time) = trunc(sysdate - 1) and NOTIFICATION_STATUS=2
/
spool off;
exit;
EOF`"
sed -i '1s/^/IMSI ,MSISDN ,NOTIFICATION_TIME ,NOTIFICATION_TEXT_EN.  /' /home/dreadmin/scripts/wresult.csv ;
echo "Hello Gord,

Please find the attached WebException error occurred while sending SMS for the date `date '+%Y-%m-%d' -d '24 hour ago'`" | mailx -s "Speed pass purchase exceptions: `date '+%Y-%m-%d' -d '24 hour ago'`" -a /home/dreadmin/scripts/wresult.csv "raju.john@syniverse.com" ;
...
rm -rf /home/dreadmin/scripts/sresult.csv;
rm -rf /home/dreadmin/scripts/wresult.csv;